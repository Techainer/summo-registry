...
Takes the room off a recording before the decoder hears it: fans, air conditioning, keyboard, traffic through a window. Half a megabyte, and it runs over each finished sentence rather than the live audio. Worth having in a noisy room and worth leaving off in a quiet one — on clean speech from a good microphone it removes things that were not noise, and the transcript gets slightly worse rather than better.
...
| Size | 523 KB |
| Licence | Apache-2.0 |
| Runtime | `sherpa-onnx/gtcrn` |

## Where this comes from

Published by Xiaobin-Rong/gtcrn, exported for sherpa-onnx by k2-fsa.

Redistributable under its licence, so Summo mirrors it. The checksums below are what you should get from either source.

## What it costs

Memory: about 30 MB resident, 90 MB at peak. Needs at least 256 MB free.

Real-time factor, measured — below 1.0 keeps up with live audio:

| Machine | RTF |
|---|---|
| `cpu_x86_avx2_1t` | 0.102 |
| `cpu_x86_avx2_4t` | 0.114 |
...
| Name | Size | sha256 |
|---|---|---|
| `gtcrn_simple.onnx` | 523 KB | `e77603ac0c23dac3227dd2d7135b3a585cbee2679048aecfa886657d3ae1b534` |
